...
class PlaywrightRunner:
    def __init__(self):
        ...
        self.screenshot_dir = config.get_value("PLAYWRIGHT", "screenshot_path", "logs/screenshots")
    ...
    async def _run_test_async(self, url, headless=False):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                filename = f"screenshot_{url.replace('https://', '').replace('http://', '').replace('/', '_').replace('.', '_')}_{timestamp}.png"
                ...
                try:
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"error_{url.replace('https://', '').replace('http://', '').replace('/', '_').replace('.', '_')}_{timestamp}.png"
                    self.screenshot_path = os.path.join(self.screenshot_dir, filename)
                    await page.screenshot(path=self.screenshot_path)
                    self._log(f"Zrzut ekranu błędu zapisany: {self.screenshot_path}")
                except:
                    self._log("Nie udało się zrobić zrzutu ekranu błędu")
